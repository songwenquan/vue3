...
<template>
  <div id="tags-view-container" class="tags-view-container bf mb10" >
    <el-scrollbar ref="scrollPaneRef" class="tags-view-wrapper cursor-p bf fz12" >
      <router-link class="tags-view-item ml5 mt5" v-for="(tag,index) in visitedViews" :key="tag.menuUrl" :data-path="tag.menuUrl" :class="isActive(tag) ? 'active' : ''" :to="{ path: tag.menuUrl, query: tag.query, fullPath: tag.fullPath }"
                   :style="activeStyle(tag)" @click.middle="!isAffix(tag) ? closeSelectedTag(tag) : ''" @contextmenu.prevent="openMenu(tag, $event,index)">
        {{ tag.meta?.title }}
...
const {visible,name,url,left,top,selectedTag,isFirstView,isLastView,tagIndex} = toRefs(reactive({
  visible:false,
  name:'',
  url:'',
  left:0,
